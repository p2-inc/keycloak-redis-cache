...
public abstract class KeycloakModelTest {
  ...
  @ClassRule
  public static final TestRule GUARANTEE_REQUIRED_FACTORY =
      new TestRule() {
        @Override
        public Statement apply(Statement base, Description description) {
          ...
          while (testClass != Object.class) {
            st =
                Stream.concat(Stream.of(testClass.getAnnotationsByType(RequireProvider.class)), st);
            testClass = testClass.getSuperclass();
          }
          List<Class<? extends Provider>> notFound =
              st.filter(KeycloakModelTest::checkProviderAvailability)
  ...
  @Rule
  public final TestRule guaranteeRequiredFactoryOnMethod =
      new TestRule() {
        @Override
        public Statement apply(Statement base, Description description) {
          Stream<RequireProvider> st =
              Optional.ofNullable(description.getAnnotation(RequireProviders.class))
            ...
            if (only.length == 0) {
              ...
              boolean notFoundAny =
                  Stream.of(only)
                      .allMatch(
                          provider ->
                              getFactory().getProviderFactory(providerClass, provider) == null);
              if (notFoundAny) {
                return new Statement() {
                  @Override
                  public void evaluate() {
                        ...
                        cn -> {
                          try {
                            return Class.forName(
                                cn.indexOf('.') >= 0
                                    ? cn
                                    : ("io.phasetwo.keycloak.jpacache.testsuite.parameters." + cn));
                          } catch (Exception e) {
  ...
  /**
   * Runs the given {@code task} in {@code numThreads} parallel threads, each thread operating in
   * the context of a fresh {@link KeycloakSessionFactory} independent of each other thread.
   *
   * <p>Will throw an exception when the thread throws an exception or if the thread doesn't
   * complete in time.
   *
  ...
  public static void inIndependentFactories(int numThreads, int timeoutSeconds, Runnable task)
    ...
    // memorize threads created to be able to retrieve their stacktrace later if they don't
    // terminate
    ...
    try {
      ...
      Callable<?> independentTask =
          () ->
              inIndependentFactory(
                  () -> {

                    // use the latch to ensure that all caches are online while the transaction
                    // below runs to avoid a RemoteException
                    start.countDown();
                    start.await();
                    ...
                    try {
                      ...
                      // use the latch to ensure that all caches are online while the transaction
                      // above runs to avoid a RemoteException
                      // otherwise might fail with "Cannot wire or start components while the
                      // registry is not running" during shutdown
      ...
      // submit tasks, and wait for the results without cancelling execution so that we'll be able
        ...
        Arrays.stream(infos)
            .map(
                thread -> {
                  ...
                  sb.append(thread.getThreadName())
                      .append(" (")
  ...
  /**
   * Runs the given {@code task} in a context of a fresh {@link KeycloakSessionFactory} which is
   * created before running the task and destroyed afterwards.
   */
  public static <T> T inIndependentFactory(Callable<T> task) {
    if (USE_DEFAULT_FACTORY) {
      throw new IllegalStateException(
          "USE_DEFAULT_FACTORY must be false to use an independent factory");
    }
  ...
  public static void checkValidParameters() {
    Assume.assumeTrue(
        ...
        MODEL_PARAMETERS.size() > 1); // Additional parameters have to be set
  }
  ...
  protected <T> Stream<T> getParameters(Class<T> clazz) {
    return MODEL_PARAMETERS.stream()
        .flatMap(mp -> mp.getParameters(clazz))
        .filter(Objects::nonNull);
  }
  ...
  /**
   * Convenience method for {@link #inComittedTransaction(java.util.function.Consumer)} that obtains
   * realm model from the session and puts it into session context before running the {@code what}
   * task.
   */
  protected <R> R withRealm(String realmId, BiFunction<KeycloakSession, RealmModel, R> what) {
    return inComittedTransaction(
        session -> {
