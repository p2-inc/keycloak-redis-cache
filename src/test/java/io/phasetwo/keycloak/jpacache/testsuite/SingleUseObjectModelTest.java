    ...
    DefaultActionTokenKey key =
        withRealm(
            ...
            (session, realm) -> {
              ...
              notes.put("foo", "bar");
              singleUseObjectProvider.put(
                  actionTokenKey.serializeKey(), actionTokenKey.getExp() - time, notes);
    ...
    inComittedTransaction(
        session -> {
         ...
         // nullNotes.put("key1", null);    -- cannot be null . @xgp a first find
          ...
          Assert.assertNotNull(singleUseStore.get("key").get("key1")); // -- not null. different test
        });
  }
