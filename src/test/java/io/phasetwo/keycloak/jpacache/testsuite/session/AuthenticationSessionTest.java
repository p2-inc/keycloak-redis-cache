...
import static io.phasetwo.keycloak.jpacache.testsuite.session.SessionTestUtils.createClients;
            ...
            (session, realm) -> {
              RootAuthenticationSessionModel ras =
                  session
                      ...
                      .getRootAuthenticationSession(realm, rootAuthSessionId.get());
              ClientModel client = realm.getClientByClientId("test-app");
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          tabIds.forEach(
              tabId -> {
                AuthenticationSessionModel authSession =
                    rootAuthSession.getAuthenticationSession(client, tabId);
  ...
  public void testRemoveAuthSession() {
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          String tabId =
              rootAuthSession.getAuthenticationSessions().values().iterator().next().getTabId();
  ...
  public void testAuthSessionProperties() {
    String rootSessionId =
        withRealm(
            ...
            (session, realm) -> {
              ...
              authenticationSession.setExecutionStatus(
                  "test-auth", CommonClientSessionModel.ExecutionStatus.CHALLENGED);
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          AuthenticationSessionModel authSession =
              rootAuthSession.getAuthenticationSessions().values().iterator().next();
          assertThat(
              rootAuthSession.getAuthenticationSession(client, authSession.getTabId()),
              is(authSession));
          ...
          assertThat(
              authSession.getRequiredActions(),
              containsInAnyOrder("VERIFY_EMAIL", "CONFIGURE_TOTP"));
  ...
  public void testAuthSessionPropertiesTransientUsers() {
        ...
        withRealm(
            ...
            (session, realm) -> {
              LightweightUserAdapter lightweightUserAdapter =
                  new LightweightUserAdapter(session, null);
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          assertThat(authSession.getRedirectUri(), is("http://localhost:8080"));
          assertThat(
              authSession.getUserSessionNotes().entrySet(), hasSize(3)); // +1 for transient user
          assertThat(authSession.getUserSessionNotes().get("key1"), is("value1"));
          assertThat(authSession.getUserSessionNotes().get("key2"), is("value2"));
          assertThat(authSession.getAuthNote("key1"), is("val1"));
          assertThat(authSession.getAuthNote("key2"), is("val2"));
